...
for i in range(args.nr_gpu):
    with tf.device('/gpu:%d' % i):
        ...
        klds[i] = compute_mmd(tf.random_normal(shape=(args.batch_size, args.z_dim), scale=10.), z_samples[i], 2./args.z_dim)
        ...
        test_klds[i] = compute_mmd(tf.random_normal(shape=(args.batch_size, args.z_dim), scale=10.), test_z_samples[i], 2./args.z_dim)
