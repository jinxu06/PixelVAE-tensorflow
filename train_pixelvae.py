...
for i in range(args.nr_gpu):
    with tf.device('/gpu:%d' % i):
        ...
        klds[i] = compute_mmd(tf.random_normal(tf.stack([200, z_dim])), z_samples[i], 2./args.z_dim)
        ...
        test_klds[i] = compute_mmd(tf.random_normal(tf.stack([200, z_dim])), test_z_samples[i], 2./args.z_dim)
