...
for i in range(args.nr_gpu):
    with tf.device('/gpu:%d' % i):
        ...
        klds[i] = compute_mmd(tf.random_normal(tf.stack([200, args.z_dim])), z_samples[i], 2./args.z_dim)
        ...
        test_klds[i] = compute_mmd(tf.random_normal(tf.stack([200, args.z_dim])), test_z_samples[i], 2./args.z_dim)
...
def sample_from_model(sess, data=None):
    ...
    for yi in range(obs_shape[0]-obs_shape[0]//2, obs_shape[0]):
